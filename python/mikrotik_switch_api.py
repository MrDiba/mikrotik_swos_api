...
def get_Mikrotik_Hosts(url, username, password):
    query = "!dhost.b"
    method = "GET"
    response = send_Mikrotik_Rest_Method(
        url=url, query=query, method=method, username=username, password=password
    )
...
def convert_Mikrotik_Vlan_Receive(hex_string=None, string=None):
    vlan_receive = {"0x00": "Any", "0x01": "Only Tagged", "0x02": "Only Untagged"}
...
def get_Mikrotik_Links(
    url, username, password, port_number=None, port_name=None, output_only=None
):
    query = "link.b"
    method = "GET"
    response = send_Mikrotik_Rest_Method(
    ...
    enabled_ports = convert_Mikrotik_Hex_Array(
        hex_string=links["en"], pad_length=total_ports
    )
    link_active = convert_Mikrotik_Hex_Array(
        hex_string=links["lnk"], pad_length=total_ports
    )
    auto_neg_ports = convert_Mikrotik_Hex_Array(
        hex_string=links["an"], pad_length=total_ports
    )
    ...
    while port_instance < total_ports:
        object = {
            ...
            "link_speed": convert_Mikrotik_Link_Speed(
                hex_string=links["spd"][port_instance]
            ),
...
def get_Mikrotik_Vlan(
    ...
    force_vlan = convert_Mikrotik_Hex_Array(
        hex_string=vlans["fvid"], pad_length=total_ports
    )
    ...
    while port_instance < total_ports:
        object = {
            ...
            "vlan_mode": convert_Mikrotik_Vlan_Mode(
                hex_string=vlans["vlan"][port_instance]
            ),
            "vlan_receive": convert_Mikrotik_Vlan_Receive(
                hex_string=vlans["vlni"][port_instance]
            ),
    ...
    output_options = ("vlan_mode", "vlan_receive", "vlan_id")
...
def get_Mikrotik_Vlan_Config(url, username, password):
    ...
    for vlan in vlans:
        object = {
            ...
            "members": convert_Mikrotik_Hex_Array(
                hex_string=vlans[vlan_instance]["mbr"], pad_length=26
            ),
        }
        vlan_instance += 1
        output.append(object)
...
def set_Mikrotik_Switch_Port(
    url, username, password, port_number, port_name=None, enabled=True, auto_neg=True
):
    query = "link.b"
    response = send_Mikrotik_Rest_Method(
        url=url, query=query, method="GET", username=username, password=password
    )
    ...
    get_Mikrotik_Link_params = dict(url=url, username=username, password=password)
    ...
    port_auto_neg = get_Mikrotik_Links(
        **get_Mikrotik_Link_params, output_only="auto_neg"
    )
    ...
    send_Mikrotik_Rest_Method(
        url=url,
        query=query,
        method="POST",
        username=username,
        password=password,
        body=new_mikrotik_config,
    )


def set_Mikrotik_Vlan(
    url,
    username,
    password,
    port_number,
    ...
    get_Mikrotik_Vlan_params = dict(url=url, username=username, password=password)
    ...
    vlan_mode_data = get_Mikrotik_Vlan(
        **get_Mikrotik_Vlan_params, output_only="vlan_mode"
    )[array_port_number]
    vlan_receive_data = get_Mikrotik_Vlan(
        **get_Mikrotik_Vlan_params, output_only="vlan_receive"
    )[array_port_number]
    vlan_id_data = get_Mikrotik_Vlan(**get_Mikrotik_Vlan_params, output_only="vlan_id")[
        array_port_number
    ]
