...
def set_Mikrotik_Switch_Port(
    ...
    port_names = get_Mikrotik_Switch_Port(
        **get_Mikrotik_Link_params, output_only="port_name"
    )
    port_enabled = get_Mikrotik_Switch_Port(
        **get_Mikrotik_Link_params, output_only="enabled"
    )
    port_auto_neg = get_Mikrotik_Switch_Port(
...
def ports():
    module_args = dict(
        ...
        command_type=dict(
            type="str", required=False, choices=["get", "set"], default="get"
        ),
        ...
        output_only=dict(
            type="str", required=False, choices=["port_name", "enabled", "auto_neg"]
        ),
    )

    result = dict(changed=False, original_message="", ports="")
    ...
    if (
        module.params["port_number"] is not None
        and module.params["command_type"] == "set"
    ):
        ...
        module.params["port_number"] is not None
        and module.params["command_type"] == "get"
    ):
        get_mikrotik_switch_port_params["port_number"] = module.params["port_number"]
        output = get_Mikrotik_Switch_Port(**get_mikrotik_switch_port_params)
